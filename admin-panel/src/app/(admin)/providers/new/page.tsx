...
export default function NewProviderPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTimeout(() => router.push(`/providers/${result.id}`), 1000);
    } catch (e) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-semibold text-gray-900">Add New Provider</h1>
      ...
      {saveStatus === "success" && (
        <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-800">Provider created successfully. Redirecting...</p>
          </div>
        </div>
      )}
      {saveStatus === "error" && (
        <div className="mb-6 bg-red-50 border border-red-200 rounded-lg p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-800">Please fix the errors below.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input id="credentials" value={formData.credentials} onChange={(e) => handleChange("credentials", e.target.value)} className="mt-1" />
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-gray-200 bg-white rounded-lg p-6">
          <Button variant="outline" onClick={() => router.push("/providers")} disabled={saveStatus === "saving"}>
            Cancel
          </Button>
          <Button onClick={() => handleSave()} disabled={saveStatus === "saving"} className="bg-blue-600 hover:bg-blue-700 text-white">
            <Save className="w-4 h-4 mr-2" />
            {saveStatus === "saving" ? "Creating..." : "Create Provider"}
          </Button>
        </div>
      </div>
    </div>
